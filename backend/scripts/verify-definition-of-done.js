...
/**
 * Standalone Node.js script to verify Definition of Done
 * Can be run with: node scripts/verify-definition-of-done.js
 */

const { PrismaClient, ProjectType, ProjectCategory, ProjectComplexity, UserRole } = require('@prisma/client');
...
async function verifyPermitBlocking() {
  ...
  try {
    ...
    if (permit.blockingReasons && Array.isArray(permit.blockingReasons)) {
      const hasContractBlock = permit.blockingReasons.some((r) => r.type === 'CONTRACT_NOT_SIGNED');
      const hasDesignBlock = permit.blockingReasons.some((r) => r.type === 'DESIGN_NOT_APPROVED');
      const hasReadinessBlock = permit.blockingReasons.some((r) => r.type === 'READINESS_INCOMPLETE');

      if (hasContractBlock || hasDesignBlock || hasReadinessBlock) {
        results.push({
          name: 'Permit Submission Blocking',
          passed: true,
          message: `Blocking reasons correctly returned: ${permit.blockingReasons.map((r) => r.type).join(', ')}`,
        });
      } else {
        results.push({
          name: 'Permit Submission Blocking',
